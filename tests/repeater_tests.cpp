...
// TODO This test really relates to Frequency, not to Repeater.
TEST_FIXTURE(TestFixture, test_repeater_frequency_phrase)
{
	...
	CHECK_EQUAL(frequency_description(repeater1.frequency()), "every day");
	CHECK
	(	typeid(frequency_description(repeater1.frequency())) ==
		typeid(std::string)
	);
	CHECK_EQUAL
	(	frequency_description(repeater1.frequency()),
		BString("every day")
	);
	...
	CHECK_EQUAL
	(	frequency_description(repeater2.frequency()),
		"every 12 days"
	);

	Repeater repeater3(dbc);
	repeater3.set_frequency(Frequency(1, interval_type::weeks));
	CHECK_EQUAL(frequency_description(repeater3.frequency()), "every week");
	
	Repeater repeater4(dbc);
	repeater4.set_frequency(Frequency(2, interval_type::weeks));
	CHECK_EQUAL
	(	frequency_description(repeater4.frequency()),
		"every 2 weeks"
	);

	Repeater repeater5(dbc);
	repeater5.set_frequency(Frequency(3, interval_type::months));
	CHECK_EQUAL(frequency_description(repeater5.frequency()), "every 3 months");

	Repeater repeater6(dbc);
	repeater6.set_frequency(Frequency(1, interval_type::months));
	CHECK_EQUAL(frequency_description(repeater6.frequency()), "every month");

	Repeater repeater7(dbc);
	repeater7.set_frequency(Frequency(12, interval_type::months));
	CHECK_EQUAL(frequency_description(repeater7.frequency()), "every 12 months");

	Repeater repeater8(dbc);
	repeater8.set_frequency(Frequency(1, interval_type::month_ends));
	CHECK_EQUAL
	(	frequency_description(repeater8.frequency()),
	...
	CHECK_EQUAL
	(	frequency_description(repeater9.frequency()),
